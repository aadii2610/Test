...
| --- | --- |
| **Jira Ticket** | *(pending)* |
| **Feature Name** | Special Request Entry Point & CTA Redesign (QSR + TSR) |
| **Module / Layer Affected** | **UI** (`QsrMenuFragment`, `TsrMenuFragment`, `QsrModifierGroupAdapter`/TSR equivalent, `DialogExtensions.kt` special-request popup, `RegisterActivity`). No Domain or Data layer changes required — `specialRequest` and its persistence already exist end-to-end. |
| **Tech Lead / Owner** | *(pending)* |
| **Estimated Complexity** | **L1** (UI/interaction-only redesign; no schema or API contract change; logic is duplicated across two large legacy Fragments, which is the main source of effort) |
| **Initial Risk Level** | **Low–Medium** (lands on a zero-test-coverage surface; re-exercises a pre-existing dead debounce guard on the same CTA) |
| **Dependencies** | **Internal:** `TicketItem.specialRequest` / `CustomItem.specialRequest` (already persisted), `Modifier.isOptional`/`isPushed` (mandatory-modifier flags), `checkForAllRequiredAndPushedModifiers()` / `checkRequiredModifier(...)`, `DialogExtensions.showSpecialRequestPopUp`, `RegisterActivity.showSpecialRequestPopup`. **External:** none — no backend/API change required. |
...
| --- | --- | --- | --- |
| Concurrency | (I-1) `addItemWithDebounce`'s guard condition (`currentTime - lastAddedTime < 0`) can never be true — debounce is dead code (`QsrMenuFragment.kt:1551`); rapid double-tap can double-add an item, and this is directly re-entered by the new "Add & Finish" CTA | High | Duplicate ticket line items, operator-visible, requires manual correction |
| Concurrency | (I-2) Editing branch of `onBtnDoneItemClicked` has no debounce guard at all in either QSR (`QsrMenuFragment.kt:1468`) or TSR (`TsrMenuFragment.kt:1511`); double-tap on "Update & Finish" launches two concurrent IO writes | Medium | Duplicate write vs. harmless overwrite — impact unverified, needs `pushItemToDb`/update-path threading check |
| Architecture | (I-3) Special Request identified only by magic string `"Special Request"` repeated across 7 files (`QsrMenuFragment.kt`, `TsrMenuFragment.kt`, adapters, `TicketItemOperations.kt`, `ModifierRepoImpl.kt`), with `id = 999`/`9999`/`-999` sentinel values — fragile to typos/renames, no compile-time safety | Low | Maintainability only — no functional risk today |
| Concurrency | (I-4) Threading of `ticketMenuViewModel.pushItemToDb(...)` (the "new item" persistence path, `QsrMenuFragment.kt:1541` area) was not traced to a dispatcher in this pass | Medium | Unknown until verified — blocks finalizing I-1/I-2's fix design |
| Reliability | (I-5) No confirmation dialog on removing an existing special request today (silent unselect, `TsrMenuFragment.kt:2635` and QSR equivalent); PRD's new "Remove Request" doesn't specify a confirmation either | Medium | Accidental data loss, more likely once the more-discoverable "+" entry point ships |
| Reliability | (I-6) Popup's existing "Done blocks on empty text" behavior (`DialogExtensions.kt:585-594`) conflicts conceptually with the new explicit "Remove Request" action — two ways to express "no request" | Low | Inconsistent UX if both mechanisms ship side by side |
| Reliability | (I-7) Fulfilled/sent-item lock (`status == FULFILLED`) is enforced ad hoc at each call site today; the 4 new CTA states don't yet account for a 5th "locked" case | Medium | Functional gap in the PRD (not a bug) — undefined behavior on a real, already-guarded status |
| Test | (I-8) Zero test coverage across the entire modifier-validation and special-request code path, app-wide | Medium | Process/regression risk rather than an immediate functional one |

## 5. Improvement Decision Matrix (Mandatory)

| Issue | Fix in Current Iteration? (Y/N) | Justification | Backlog Ticket | Target Sprint |
| --- | --- | --- | --- | --- |
| Fix dead debounce guard (I-1) | Y | One-line condition fix; CR is already editing this exact function and makes "Add & Finish" its headline CTA, so the bug's traffic increases regardless | *[Ref]* | *[Sprint]* |
| Add debounce to editing branch (I-2) | Y | Mirrors the I-1 fix for "Update & Finish"; do after I-4 is verified so the fix accounts for the real dispatcher behavior | *[Ref]* | *[Sprint]* |
| Verify `pushItemToDb` / update-path dispatcher (I-4) | Y | Read-only verification task; blocks finalizing I-1/I-2's implementation, must complete first | *[Ref]* | *[Sprint]* |
| Replace magic-string identification with a typed model (I-3) | N | Medium–high effort across 7 files in both flows; broad surface area risks regressing unrelated modifier code (Item Comps, custom modifiers) for a CR that should stay UX-scoped | *[Ref]* | *[Sprint]* |
| Retire "Done blocks on empty" in favor of explicit Remove (I-6) | Y | The PRD's 4 states assume Remove is the only removal path — required for the CR to be internally consistent | *[Ref]* | *[Sprint]* |
| Confirmation dialog on Remove Request (I-5) | Y (pending product) | Product decision, not purely engineering — PRD is silent on it; recommend requiring one given the more discoverable entry point | *[Ref]* | *[Sprint]* |
| Define behavior for fulfilled/locked items (I-7) | Y (pending product) | Needed to fully specify the CTA-state derivation function; likely "+" is hidden/disabled on fulfilled items matching today's tile-level guard, but needs confirmation | *[Ref]* | *[Sprint]* |
| Extract CTA-state derivation into a pure, unit-tested function (I-8, scoped) | Y | New code should not inherit the surrounding Fragment's test-coverage exemption; scoped narrowly, not a general backfill | *[Ref]* | *[Sprint]* |
| Consolidate QSR/TSR special-request handling into one shared component | N | High effort/risk — both Fragments are ~3000-line God Fragments with divergent surrounding logic (breadcrumb state in TSR, none in QSR); out of scope for a UX-focused CR | *[Ref]* | *[Sprint]* |
| Confirmation dialog on Remove Request (I-5) | Low (reuse existing confirm-dialog pattern elsewhere in the app) | Low–Medium — accidental data loss, but request text is a soft-value note, not money/payment data | Low | **Product decision, not engineering** | **Escalate to product/design for an explicit answer before implementation**, since the PRD is silent on it and the new entry point is by design more discoverable/tappable. |
| Retire "Done blocks on empty" in favor of explicit Remove (I-6) | Low (delete one branch, rely on the new secondary button) | Medium — leaving both creates confusing, inconsistent UX exactly in the flow this CR is redesigning | Low | **Yes** — the PRD's 4 states assume Remove is the only removal path | **Bundle** — required for the CR to be internally consistent. |
| Define behavior for fulfilled/locked items (I-7) | Low (extend existing status guard into the new state derivation) | Medium — undefined behavior on a real, already-guarded status value | Low | **Yes**, needs an answer before the CTA-state derivation function can be written completely | **Resolve with product before implementation** — likely "+" affordance is hidden/disabled on fulfilled items, matching today's tile-level guard, but confirm explicitly. |
| Extract CTA-state derivation into a small pure function + unit tests (I-8, scoped) | Low | Medium — this is new logic; shipping it untested on a zero-coverage surface compounds the existing gap | Low | No | **Bundle**, scoped narrowly to the new 4-state derivation only — not a general test-backfill effort. |
| Consolidate QSR/TSR special-request handling into one shared component | High (both Fragments are ~3000-line God Fragments with divergent surrounding logic — breadcrumb state in TSR, none in QSR) | Low — current duplication is stable, just verbose | High — large, cross-cutting refactor risk for a CR that is supposed to be UX-scoped | No | **Defer.** At minimum, do not implement the new modal twice from scratch — extract *only* the modal + its CTA-state logic into one shared class/function called from both Fragments, without attempting to unify the surrounding Fragment architecture. |
...
| --- | --- | --- | --- |
| Double-tap on "Add & Finish"/"Update & Finish" duplicates a ticket item or fires duplicate updates (I-1, I-2) | Medium | High | Bundle the debounce fix per Decision Matrix; verify dispatcher behavior first (I-4) |
| "Return to Modifier screen" branch doesn't restore the pushed/required-modifier highlight state the PRD promises ("It will highlight when you return") | Medium | Medium | Implement the "return to Modifier" branch by calling the existing `getModifierListView`/`checkModifierView` functions rather than new bespoke navigation code |
| Accidental request removal via the more-discoverable "+" entry point, with no confirmation | Medium | Low | Escalate I-5 to product for an explicit confirmation-dialog decision before implementation |
| Undefined behavior on fulfilled/locked items under the new 4-state model | Medium | Medium | Resolve I-7 with product before writing the state-derivation function |
| New logic ships untested on an already zero-coverage surface | High | Low–Medium | Governance Rule 4 — new CTA-state derivation must be a unit-tested pure function |
| Scope creep into the God-Fragment architecture or magic-string cleanup (I-3, consolidation) | Medium | High | Governance Rules 1–2 explicitly gate this; both are logged as deferred follow-ups, not silently absorbed |
